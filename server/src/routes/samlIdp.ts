...
const IDP_ENTITY_ID = process.env.IDP_ENTITY_ID ?? 'https://sgummalla-net.fly.dev'
const SP_ACS_URL = process.env.SP_ACS_URL ?? 'https://support.sgummalla.net/login'
const SP_ENTITY_ID = process.env.SP_ENTITY_ID ?? 'https://support.sgummalla.net'
...
router.get('/sso', async (req: Request, res: Response) => {
  ...
  if (!token) {
    ...
    res.redirect(`${process.env.CLIENT_URL ?? 'http://localhost:5173'}/login?${params.toString()}`)
    return
  }
  ...
  try {
    ...
    try {
      ...
      console.warn('[vZen SAML IDP] Could not parse SAMLRequest ID, using random')
    }
...
<body onload="document.forms[0].submit()">
  <form method="POST" action="${SP_ACS_URL}">
    ...
    console.error('[vZen SAML IDP]', err)
...
<md:EntityDescriptor xmlns:md="urn:oasis:names:tc:SAML:2.0:metadata" entityID="${IDP_ENTITY_ID}">
  <md:IDPSSODescriptor WantAuthnRequestsSigned="false" protocolSupportEnumeration="urn:oasis:names:tc:SAML:2.0:protocol">
    ...
    res.status(500).json({ error: 'Failed to generate IDP metadata', detail: msg })
  }
})
