...
<div class="container">
  ...
  <div class="section">
    ...
    <p class="section-desc">As of Spring '26, Experience Cloud supports two distinct site runtimes. They differ in security architecture, LWS behaviour, and CSP enforcement. Every statement in this document about LWC constraints applies to <strong>both runtimes</strong> unless explicitly noted otherwise.</p>
    <div class="data-grid" style="grid-template-columns: 1.4fr 1fr 1fr;">
      ...
      <div class="cell center"><span class="yes">Globals exposed directly</span><span class="note-small"><code>window</code>, <code>document</code>, element globals accessible without secure wrappers — within namespace sandbox only</span></div>
      ...
      <div class="cell dim last-row">Visualforce component support in Experience Builder</div>
      <div class="cell center last-row"><span class="yes">✓ Supported</span><span class="note-small">VF page component available on both runtimes</span></div>
      <div class="cell center last-col last-row"><span class="yes">✓ Supported</span></div>
    </div>
    <div class="callout warning">
      <strong>The postMessage problem exists on both runtimes.</strong> Although LWR's LWS exposes <code>window</code> globals directly within a namespace sandbox, an external third-party iframe is a cross-origin browsing context — outside any Salesforce namespace entirely. Messages posted from the iframe arrive at the real browser <code>window</code>, not the namespace-scoped object the LWC component holds. The widget communication failure described in Section 02 applies equally to LWR and Aura sites.
    </div>
  </div>
  ...
  <div class="section">
    ...
    <p class="section-desc">Most third-party widgets delivered as an embeddable script share the same five hard requirements. Each requirement constrains which Salesforce integration mechanism can actually support it.</p>
    <div class="data-grid" style="grid-template-columns: 1.9fr 1fr 1fr 1fr;">
      ...
      <div class="cell center"><span class="partial">⚠ Partial</span><span class="note-small">loadScript fetches it, but LWS sandboxes execution</span></div>
      <div class="cell center"><span class="yes">✓ Yes</span><span class="note-small">Standard &lt;script src&gt; in VF page head</span></div>
      <div class="cell center last-col"><span class="yes">✓ Yes</span><span class="note-small">Script runs on your own server-rendered page</span></div>
      <div class="cell dim">Execute that script outside the LWS sandbox</div>
      <div class="cell center"><span class="no">✗ No</span><span class="note-small">All LWC code — including loadScript — runs inside the LWS compartment</span></div>
      <div class="cell center"><span class="yes">✓ Yes</span><span class="note-small">VF iframe is a completely separate browsing context</span></div>
      <div class="cell center last-col"><span class="yes">✓ Yes</span><span class="note-small">Canvas iframe is a separate browsing context</span></div>
      <div class="cell dim">window.postMessage works correctly (widget ↔ host page events, minimize/maximize)</div>
      <div class="cell center"><span class="no">✗ No</span><span class="note-small">Proxy window ≠ real window; listeners register on the wrong object</span></div>
      <div class="cell center"><span class="yes">✓ Yes</span><span class="note-small">Real window inside VF iframe context</span></div>
      <div class="cell center last-col"><span class="yes">✓ Yes</span><span class="note-small">Canvas JS SDK is postMessage-based; works correctly</span></div>
      <div class="cell dim">position: fixed floating button that overlays the full EC page</div>
      <div class="cell center"><span class="no">✗ No</span><span class="note-small">Script execution is broken anyway — moot</span></div>
      <div class="cell center"><span class="yes">✓ Yes</span><span class="note-small">Full-screen transparent iframe overlay; native behaviour</span></div>
      <div class="cell center last-col"><span class="partial">⚠ Partial</span><span class="note-small">Canvas iframe is a layout component; overlay needs a workaround</span></div>
      <div class="cell dim last-row">Per-user access token fetched from the server and passed to the widget on mount</div>
      <div class="cell center last-row"><span class="partial">⚠ Partial</span><span class="note-small">@wire can fetch a token, but the widget cannot consume it across the LWS boundary</span></div>
      <div class="cell center last-row"><span class="yes">✓ Yes</span><span class="note-small">Apex controller + {!mergeField} — server-rendered, no async step</span></div>
      <div class="cell center last-col last-row"><span class="yes">✓✓ Yes</span><span class="note-small">Signed Request — cleaner; no Apex needed</span></div>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <p class="section-desc">On <strong>Aura sites</strong>, LWS wraps all globals in membrane proxies — components never touch real browser objects. On <strong>LWR sites</strong>, LWS is less restrictive: <code>window</code>, <code>document</code>, and element globals are exposed directly within the namespace sandbox, without secure wrappers. However, neither model solves the external widget problem. A third-party iframe is a cross-origin browsing context that sits entirely outside the Salesforce namespace — no amount of relaxed global access inside the LWC compartment bridges that boundary.</p>
...
│   └── loadScript(widget.js)  →  executes inside namespace sandbox → same scoped globals
  ...
  // eval(), Function(), dynamic script injection, frame reference tricks — all intercepted on both runtimes.</span></pre>
    ...
    <div class="data-grid" style="grid-template-columns: 1.6fr 0.9fr 2fr;">
      ...
      <div class="cell last-col last-row">Depends on postMessage from host into the widget iframe</div>
    </div>

    <div class="callout danger">
      <strong>Without Trusted Mode, there is no supported escape path from LWS.</strong> <code>eval()</code>, <code>Function()</code>, <code>document.createElement('script')</code>, and frame reference tricks (<code>window[0]</code>, <code>window.frames[0]</code>) are all intercepted on both runtimes. Salesforce actively closes these as they are discovered. The <code>loadScript</code> path — pointing at an <strong>external URL</strong> — remains a dead end: external scripts loaded by URL still execute inside the LWS sandbox regardless of which runtime the site uses.
    </div>
    <div class="callout warning">
      <strong>LWS Trusted Mode (Winter '26 GA, Spring '26) — a conditional path opens for LWC.</strong>
      Trusted Mode allows third-party scripts loaded via <code>loadScript</code> to bypass LWS and Locker entirely, giving unrestricted access to the real <code>window</code>, <code>document</code>, and DOM globals. This directly fixes the <code>postMessage</code> problem — <code>window.addEventListener('message', handler)</code> registers on the real browser window, so widget bidirectional communication works correctly.
      <br><br>
      <strong>However, Trusted Mode only applies to static resources — not external script URLs.</strong> The widget bundle must be uploaded to Salesforce as a Static Resource and whitelisted by an admin in Session Settings. Static Resources have a hard 5MB per-file size limit. If your widget bundle exceeds 5MB it cannot be uploaded, and Trusted Mode does not apply.
      <br><br>
      <strong>Viability for LWC in Spring '26:</strong><br>
      ...
      <span style="color:var(--yellow)">⚠ Caveat</span> — Official Salesforce docs still state Experience Builder sites don't fully support third-party components when LWS is enabled. Test thoroughly in a sandbox before production.
    </div>
  </div>
  ...
  <div class="section">
    ...
    <p class="section-desc">A Visualforce page runs in the classic Salesforce rendering pipeline — an entirely separate browsing context that LWS never touches. Embedded as a transparent, <code>pointer-events: none</code> iframe covering the full Experience Cloud page, it provides a true fixed overlay without interfering with the underlying page content.</p>
    <div class="two-col">
    ...
    <div class="detail-card">
      <div class="detail-card-header"><span class="badge badge-vf">Apex Controller</span><h3>Server-side token generation — token baked into the page before delivery</h3></div>
...
&lt;<span class="tag">apex:page</span> <span class="attr">controller</span>=<span class="str">"WidgetController"</span> <span class="attr">showHeader</span>=<span class="str">"false"</span> <span class="attr">sidebar</span>=<span class="str">"false"</span> <span class="attr">standardStylesheets</span>=<span class="str">"false"</span>&gt;
    ...
    &lt;<span class="tag">script</span>&gt;
        window.<span class="fn">mountWidget</span>({ server: <span class="str">"https://widget.yourserver.com"</span>, accessToken: <span class="str">"{!accessToken}"</span>, theme: <span class="str">"dark"</span> });
    &lt;/<span class="tag">script</span>&gt;
&lt;/<span class="tag">apex:page</span>&gt;</pre>
      </div>
    </div>
    <div class="detail-card">
      <div class="detail-card-header"><span class="badge badge-vf">Overlay CSS</span><h3>Full-screen transparent iframe — page content remains fully interactive</h3></div>
      <div class="detail-card-body">
        <pre class="code-block">.<span class="fn">vf-widget-frame</span> {
    ...
    <div class="detail-card">
      <div class="detail-card-header"><span class="badge badge-vf">JS Remoting</span><h3>On-demand token refresh without a page reload</h3></div>
      <div class="detail-card-body">
        <pre class="code-block">Visualforce.remoting.Manager.<span class="fn">invokeAction</span>(
    ...
    <span class="kw">function</span>(result, event) {
        <span class="kw">if</span> (event.status) {
            window.<span class="fn">mountWidget</span>({ server: <span class="str">"https://widget.yourserver.com"</span>, accessToken: result, theme: <span class="str">"dark"</span> });
        }
    },
    ...
    <div class="callout success"><strong>Guest user support:</strong> Because authentication runs through Apex, this pattern works for unauthenticated Experience Cloud pages. For guest users, the Apex method can return a guest-scoped token, a public session token, or redirect to login — the choice is yours.</div>
  </div>

  <div class="section">
    <div class="section-label">04 — Option B</div>
    <div class="section-title">Salesforce Canvas App</div>
    <p class="section-desc">Canvas is Salesforce's native mechanism for embedding an externally-hosted web application via a managed iframe. The defining feature is its <strong>Signed Request</strong> authentication flow — Salesforce delivers a cryptographically signed user context to your server on every page load, eliminating the need to write any Apex token-generation code.</p>
    <div class="two-col">
      <div class="pros-cons pros">
        <h4>Advantages</h4>
        <ul>
      ...
      <div class="pros-cons cons">
        ...
        <ul>
          ...
          <li>Users must be pre-authorised via profile or permission set assignment</li>
    ...
    <div class="detail-card">
      <div class="detail-card-header"><span class="badge badge-canvas">Signed Request Flow</span><h3>How Canvas delivers verified user identity to your server</h3></div>
      <div class="detail-card-body">
        <div class="flow-steps">
          <div class="flow-step"><div class="step-num">1</div><div class="step-content"><strong>User loads the Experience Cloud page</strong><p>The Canvas component renders in the Experience Builder layout.</p></div></div>
          <div class="flow-step"><div class="step-num">2</div><div class="step-content"><strong>Salesforce POSTs a signed request to your Canvas App URL</strong><p>Payload is HMAC-SHA256 signed with your Connected App secret. Contains user ID, email, org ID, OAuth access token, and the community URL.</p></div></div>
          <div class="flow-step"><div class="step-num">3</div><div class="step-content"><strong>Your server verifies the signature</strong><p>Base64-decode the payload, recompute the HMAC, compare. If valid, extract the user context.</p></div></div>
          <div class="flow-step"><div class="step-num">4</div><div class="step-content"><strong>Your server generates an access token for the verified user</strong><p>You have cryptographically confirmed user identity at this point — sign and return the token.</p></div></div>
          <div class="flow-step"><div class="step-num">5</div><div class="step-content"><strong>Server renders the page that mounts the widget</strong><p>The widget mount call is made inline with the token. No Apex, no async fetch, no race condition.</p></div></div>
        </div>
        <pre class="code-block"><span class="kw">function</span> <span class="fn">verifyAndDecode</span>(signedRequest, consumerSecret) {
    <span class="kw">const</span> [encodedSig, encodedPayload] = signedRequest.<span class="fn">split</span>(<span class="str">'.'</span>);
    <span class="kw">const</span> expectedSig = crypto.<span class="fn">createHmac</span>(<span class="str">'sha256'</span>, consumerSecret).<span class="fn">update</span>(encodedPayload).<span class="fn">digest</span>(<span class="str">'base64'</span>);
    <span class="kw">if</span> (expectedSig !== encodedSig) <span class="kw">throw new</span> Error(<span class="str">'Signature mismatch — request is not from Salesforce'</span>);
    <span class="kw">return</span> JSON.<span class="fn">parse</span>(Buffer.<span class="fn">from</span>(encodedPayload, <span class="str">'base64'</span>).<span class="fn">toString</span>(<span class="str">'utf8'</span>));
}</pre>
      </div>
    </div>
    <div class="detail-card">
      <div class="detail-card-header"><span class="badge badge-canvas">Overlay Workarounds</span><h3>Canvas iframes are layout components — three paths to a floating button</h3></div>
      <div class="detail-card-body">
        <div class="flow-steps">
          <div class="flow-step"><div class="step-num" style="background:var(--yellow); color:#000;">A</div><div class="step-content"><strong>Full-page Canvas iframe (simplest)</strong><p>Size the Canvas component to fill the full page content area. The widget's <code>position: fixed</code> is relative to the iframe's own viewport — it appears to float over the page.</p></div></div>
          <div class="flow-step"><div class="step-num">B</div><div class="step-content"><strong>Canvas + postMessage to a coordinating LWC (best visual accuracy)</strong><p>The Canvas JS SDK sends a postMessage to the Experience Cloud parent page. A thin LWC listens and renders the floating button natively in the EC DOM. The LWC only handles the toggle signal — the widget script never runs through LWS.</p></div></div>
          <div class="flow-step"><div class="step-num" style="background:var(--text-muted);">C</div><div class="step-content"><strong>Use Canvas for auth only; Visualforce handles the overlay (hybrid)</strong><p>The Canvas Signed Request vends the access token to your server. That token is then passed to a separately-embedded VF overlay page that mounts the widget.</p></div></div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <p class="section-desc">In Experience Builder → Settings → Advanced → Head Markup, you can inject a <code>&lt;script src&gt;</code> tag that loads into the page's actual <code>&lt;head&gt;</code>, outside LWS.</p>
    <div class="callout warning"><strong>Access token delivery is the problem.</strong> The Head Markup script executes before any LWC has initialised, so <code>@wire</code> and Apex are not available synchronously. You must either (a) fetch the token asynchronously from an Apex REST endpoint, or (b) rely on a separate LWC to call the mount function on the parent <code>window</code> — but that call crosses the LWS boundary.</div>
    <pre class="code-block">&lt;<span class="tag">script</span> <span class="attr">src</span>=<span class="str">"https://widget.yourserver.com/widget/index.js"</span> defer&gt;&lt;/<span class="tag">script</span>&gt;
&lt;<span class="tag">script</span>&gt;
    <span class="fn">fetch</span>(<span class="str">'/services/apexrest/WidgetToken'</span>, { headers: { <span class="str">'Authorization'</span>: <span class="str">'Bearer '</span> + userInfo.sessionId } })
    .<span class="fn">then</span>(r =&gt; r.<span class="fn">json</span>())
    .<span class="fn">then</span>(data =&gt; window.<span class="fn">mountWidget</span>({ server: <span class="str">"https://widget.yourserver.com"</span>, accessToken: data.token, theme: <span class="str">"dark"</span> }));
&lt;/<span class="tag">script</span>&gt;</pre>
    <p style="color:var(--text-dim); font-size:13px; margin-top:8px;">Head Markup works best as the <em>script loader</em> while VF or Canvas handles authentication.</p>
  </div>
  ...
  <div class="section">
    ...
    <div class="data-grid" style="grid-template-columns: 1.7fr 1fr 1fr 1fr 1fr;">
      ...
      <div class="cell center"><span class="partial">⚠</span><span class="note-small">@wire works; passable if Trusted Mode enables real window access</span></div>
      ...
      <div class="cell center">No</div>
      <div class="cell center last-col">No</div>
      <div class="cell dim">Setup complexity</div>
      <div class="cell center" style="color:var(--yellow);">Medium–High<span class="note-small">Trusted Mode config + static resource + admin approval</span></div>
      <div class="cell center" style="color:var(--green);">Low</div>
      <div class="cell center" style="color:var(--red);">High<span class="note-small">Connected App + sig verify</span></div>
      <div class="cell center last-col" style="color:var(--yellow);">Medium</div>
      <div class="cell dim">Apex required</div>
      <div class="cell center">No</div>
      <div class="cell center">Yes <span class="note-small">(simple)</span></div>
      <div class="cell center">No</div>
      ...
      <div class="cell center last-row" style="color:var(--green);">High<span class="note-small">LWC + Trusted Mode is the emerging supported path</span></div>
      <div class="cell center last-row" style="color:var(--yellow);">Stable<span class="note-small">Mature, not evolving</span></div>
      <div class="cell center last-row" style="color:var(--yellow);">Stable<span class="note-small">Mature, not evolving</span></div>
      <div class="cell center last-col last-row" style="color:var(--yellow);">Stable</div>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="decision-tree">
      ...
      <div class="dt-row"><div class="dt-condition"><strong>Want the best of both</strong> — Canvas's clean auth model plus VF's native overlay — and willing to manage two components</div><div class="dt-arrow">→</div><div class="dt-result hybrid">Canvas auth + VF overlay (hybrid)</div></div>
    ...
    <div class="recommendation">
      ...
      <p style="margin-top:16px; color:var(--text-dim); font-size:13px; line-height:1.7;">The VF approach remains the most straightforward and universally compatible path. Canvas is the cleanest for authentication when guest users aren't needed. LWC + Trusted Mode is the most platform-native option and the likely long-term direction Salesforce is heading — but it is conditional on bundle size and carries a sandbox-first requirement given its newness.</p>
    </div>
    <div class="callout info">
      <strong>Trusted URLs — required regardless of approach.</strong> The legacy "CSP Trusted Sites" setting was renamed to <strong>Trusted URLs</strong> in Summer '24 (API v58+) and now covers both CSP directives and Permissions-Policy directives. Navigate to Setup → <strong>Trusted URLs</strong> and add your widget server's origin with <code>script-src</code>, <code>connect-src</code>, and <code>frame-src</code> enabled. Set the CSP Context to <strong>All</strong>. On LWR sites, the site-level Experience Builder setting controls LWS independently of the org-level Session Settings toggle — verify it is correctly configured after any upgrade.
    </div>
  </div>

  <div class="section">
    <div class="section-label">08 — 2026 Security Warning</div>
    <div class="section-title">Third-Party Cookie Restrictions &amp; CSP Directive Selection</div>
    <p class="section-desc">Two security considerations are frequently underestimated during implementation and become production problems after launch. Both apply to every integration approach described in this document.</p>
    ...
    <div class="detail-card">
      ...
      <div class="detail-card-body">
        <p style="color:var(--text-dim); font-size:15px; margin-bottom:16px;">Google cancelled its forced Chrome deprecation in April 2025 and deprecated all Privacy Sandbox replacement APIs in October 2025. Third-party cookies remain enabled by default in Chrome — but this is not the full picture.</p>
        ...
        <div class="callout warning" style="margin-top:16px;"><strong>Safari + Firefox represent roughly 30% of browser users in 2026.</strong> Any widget that relies on its own server-side session cookie being readable inside the Salesforce iframe will silently break for these users — no error, just an unauthenticated or blank widget state.</div>
        ...
        <div class="data-grid" style="grid-template-columns: 1fr 2fr;">
          ...
          <div class="cell last-col">JWT is passed as a JavaScript parameter to the widget mount call — never stored in a cookie for auth. This sidesteps the cookie problem for authentication entirely. <strong>Risk:</strong> if the widget server also sets a session cookie for conversation state or analytics, that cookie needs <code>SameSite=None; Secure; Partitioned</code> (CHIPS) to survive Safari and Firefox.</div>
          <div class="cell dim">LWC + Trusted Mode</div>
          <div class="cell last-col">Same as above — token is passed via <code>loadScript</code> parameters. If the widget makes subsequent authenticated requests using <code>fetch</code> with credentials, those requests must use token headers, not cookies, for Safari/Firefox compatibility.</div>
          <div class="cell dim last-row">Any approach — widget using <code>localStorage</code> or <code>sessionStorage</code></div>
          <div class="cell last-col last-row">Chrome partitioned storage in cross-site iframes since Fall 2023. Safari and Firefox also partition storage. State stored in <code>localStorage</code> inside the widget iframe is partitioned per Salesforce origin — acceptable for most widget state, but users on multiple Salesforce community URLs won't share state across those URLs.</div>
        </div>
      </div>
    </div>
    ...
    <div class="detail-card">
      ...
      <div class="detail-card-body">
        <p style="color:var(--text-dim); font-size:15px; margin-bottom:16px;">In Setup → Trusted URLs, each directive unlocks a specific browser resource type. Enabling too few causes partial, hard-to-diagnose failures. The correct set for a third-party widget integration is precisely these three:</p>
        <div class="data-grid" style="grid-template-columns: 0.7fr 1fr 1.4fr 0.9fr;">
          ...
          <div class="cell">Allows the widget bundle to be fetched and executed — whether via <code>&lt;script src&gt;</code> in a VF page, <code>loadScript</code> in LWC, or Head Markup injection</div>
          <div class="cell last-col">Browser blocks script load. Widget never initialises.</div>
          ...
          <div class="cell last-row">Only needed if the widget loads these resources directly from its own origin rather than bundling them. Check the browser's Network tab — add only what is actually requested.</div>
          <div class="cell last-col last-row">Resource blocked silently. Broken icons or missing fonts in the widget UI.</div>
        </div>
        <div class="callout info" style="margin-top:16px;"><strong>Set CSP Context to "All" for each entry.</strong> Scoping a directive to only "Lightning Experience" or "Visualforce" will block it in the Experience Cloud site context, which is distinct from both. Use <strong>All</strong> to cover Experience Cloud, Lightning Experience, and Visualforce simultaneously. After saving, use the browser's DevTools Network tab to confirm no CSP violations appear on page load.</div>
        <div class="callout warning"><strong>The <code>frame-ancestors</code> directive must be set on your server — it cannot be configured in Salesforce's Trusted URLs.</strong> <code>frame-ancestors</code> is an outbound header that your widget server sends to tell browsers which origins are allowed to embed it in an iframe. Trusted URLs only control inbound CSP for the Salesforce page. Both sides must be configured: Salesforce allows the iframe via <code>frame-src</code>, and your server permits being embedded via <code>frame-ancestors</code>.</div>
      </div>
    </div>
  </div>
